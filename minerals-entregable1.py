...
from openpyxl.formatting.rule import FormatObject, DataBar, Rule, CellIsRule, DataBarRule
...
SPECIFIC_MINERALS = sorted([
    "Albite", "Amphibole", "Apatite", "Andesine", "Biotite", "Biotite Ti", "Carbonate-fluorapatite", "Ilmenite",
    "K-Feldespar", "Muscovite", "Orthoclase", "Plagioclase", "Qz-Fe", "Quartz", "Rutile", "Titanite",
    
])

# Lista de minerales secundarios ordenados alfabéticamente
SECONDARY_MINERALS = sorted([
    "Actinolite", "Alunite", "Anhydrite", "Ankerite", "Andradite", "Calcite", "Chlorite", "Clay", "Clay_Mg", "Clay_Ca",
    "Diopside", "Dolomite", "Dravite", "Epidote", "Gree-Gray Sericite", "Gypsum", "Halloysite",
    "Hastingsite", "Hornblende", "Indeterminate Clay", "Kaolinite", "Montmorillonite", "Phengite",
    "Qz-Ser", "Sericite", "Smectite", "Smithsonite", "Tremolite"
])
...
ELEMENTS = sorted([
    ...
    "Cl", "Cm", "Cn", "Co", "Cr", "Cs", "Cu", "Db", "Ds", "Dy", "Er", "Es", "Eu", "F", "Fe", "Fl", "Fm", "Fr", "Ga",
    "Gd", "Ge", "H", "He", "Hf", "Hg", "Ho", "Hs", "I", "In", "Ir", "K", "Kr", "La", "Li", "Lr", "Lu", "Lv", "Mc",
    "Md", "Mg", "Mn", "Mo", "Mt", "N", "Na", "Nb", "Nd", "Ne", "Nh", "Ni", "No", "Np", "O", "Og", "Os", "P", "Pa",
    "Pb", "Pd", "Pm", "Po", "Pr", "Pt", "Pu", "Ra", "Rb", "Re", "Rf", "Rg", "Rh", "Rn", "Ru", "S", "Sb", "Sc", "Se",
    "Sg", "Si", "Sm", "Sn", "Sr", "Ta", "Tb", "Tc", "Te", "Th", "Ti", "Tl", "Tm", "Ts", "U", "V", "W", "Xe", "Y",
    "Yb", "Zn", "Zr"
])
...
def process_mineral_data(df):
    ...
    if total_presence > 0:
        ...
        # Fusionar "Biotite" y "Biotite Ti" bajo "Biotite"
        if "Biotite Ti" in mineral_presence:
            if "Biotite" in mineral_presence:
                mineral_presence["Biotite"] += mineral_presence.pop("Biotite Ti")
            else:
                mineral_presence["Biotite"] = mineral_presence.pop("Biotite Ti")

        # Filtrar solo los minerales específicos y secundarios
        row.update({k: v for k, v in mineral_presence.items() if k in SPECIFIC_MINERALS or k in SECONDARY_MINERALS})
        # Incluir todos los demás minerales que no están en los grupos específicos o secundarios
        other_minerals = {k: v for k, v in mineral_presence.items() if k not in SPECIFIC_MINERALS and k not in SECONDARY_MINERALS and k not in ELEMENTS}
...
def format_worksheet(ws, combined_df, existing_elements, mineral_colors):
    ...
    # Convertir los colores en mineral_colors a formato aRGB
    for key in mineral_colors:
        mineral_colors[key] = hex_to_argb(mineral_colors[key])
    ...
    secondary_fill_color = PatternFill(start_color=hex_to_argb("#FFC9FF"), end_color=hex_to_argb("#FFC9FF"), fill_type="solid")
    tertiary_fill_color = PatternFill(start_color=hex_to_argb("#FFE699"), end_color=hex_to_argb("#FFE699"), fill_type="solid")
    element_fill_color = PatternFill(start_color=hex_to_argb("#C6E0B4"), end_color=hex_to_argb("#C6E0B4"), fill_type="solid")
...
def main():
    ...
    def on_ok():
        ...
        # Verificar y filtrar solo las columnas de los minerales específicos, secundarios y elementos químicos que están presentes en el DataFrame
        ...
        excluded_minerals = [col for col in combined_df.columns if col not in ["Hole ID", "From", "To"] + existing_primary_minerals + existing_secondary_minerals + existing_elements]
        
        # Reorganizar las columnas: primero los específicos, luego secundarios, luego elementos, luego excluidos
        specific_columns = ["Hole ID", "From", "To"] + existing_primary_minerals + existing_secondary_minerals + existing_elements + excluded_minerals
        ...
        with pd.ExcelWriter("Entregable Proyecto 1.xlsx", engine="openpyxl") as writer:
        ...
        print("Archivo Excel 'Entregable Proyecto 1.xlsx' generado exitosamente.")
